...
데이터의 feature이 매우 적고, 예시로 만든 데이터에 불과하지만, SGD가 더 빠르게 수렴하는것을 확인할 수 있다. 
더해서 수렴 속도의 차이를 확인 할 수 있도록, SGD와 GD의 Contour line 도 그려보았다.
...
plt.legend()
```
![image](https://user-images.githubusercontent.com/75593825/136521437-2278e483-4488-442f-b3f8-d42b918ac51b.png)

이처럼 SGD는 수렴만 할 수 있다면, 매우 빠르기 때문에 주로사용되는 기법 중 하나라고 한다.
하지만, SGD는 하나의 데이터마다 update를 진행하기 때문에, alpha값에 매우 불안정하고, 데이터에 따라 수렴하지 않는 경우도 매우 많다고한다. 따라서 Step size를 조절하는 다양한 기법들이 존재하고, 기회가 된다면 더 다루어보기로 하겠다.


추가로, SGD를 구현할때는 Backtracking을 구현하지 않았는데, 이유는 다음과 같다. [위키피디아](https://en.wikipedia.org/wiki/Stochastic_gradient_descent)Backtracking 부분을 해석해보면, Backtracking을 사용하는 이유는 올바른 stepsize를 매 iteration마다 찾기 위함인데, SGD에 backtracking을 사용하는것은 SGD가 가질 수 있는 속도라는 장점을 무효화시키는것이나 마찬가지이기 때문이다. 

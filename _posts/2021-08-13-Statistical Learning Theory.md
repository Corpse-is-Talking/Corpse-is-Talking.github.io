...
__Bayes decision function $f^*:X\rightarrow A$ is a function that achieves minimal risk among all possible function.__ 
...
평균이 $m$인 분포 $z$ 로부터 $i.i.d$하게 $z_1, z_2,...z_n$ 을 추출했다고 하면, $$\underset{n\rightarrow \infty}{\operatorname{lim}} \sum_{n=1}^{\infty} z_i = m$$
을 만족하고, 이를 큰 수 의 법칙이라고 한다.
즉, 특정 분포를 가진 모집단으로부터 데이터를 뽑는 경우에, 데이터를 많이 뽑을수록  표본평균은 원래 분포의 평균에 가까워진다는 것이다.

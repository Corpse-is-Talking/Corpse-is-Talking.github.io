...
'''
위에 주석처리된 코드는 순수하게 조건문하나 없이 Brute Force방법으로 한것이고, 타임 리밋을 페스하지 못했다. 브루트 포스를 작성하고나니, 코드의 효율성에서 문제가 생겼는데 이건 다음코드를 보면서 설명을 하겠습니다.
...
````
이 코드는 양쪽 끝에서 시작하여, 양쪽에서 선택된 컨테이너들 중에서, 높이가 작은쪽을 이동시키고, 큰쪽은 그대로 놔두는 알고리즘이다. $O(n^2)$의 복잡도를 가지며, 코드가 훨씬 간결해지고, 빨라졌다. 
